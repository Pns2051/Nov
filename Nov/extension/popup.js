  ...
  function showOnboardingView() {
    ...
    chrome.runtime.sendMessage({ action: 'getOS' }, (info) => {
      ...
      if (info.os === 'mac' || info.os === 'linux') {
        command = `curl -fsSL https://cdn.jsdelivr.net/gh/${githubUser}/Nov@latest/install.sh | bash`;
      } else if (info.os === 'win') {
        command = `iwr -useb https://cdn.jsdelivr.net/gh/${githubUser}/Nov@latest/install.ps1 | iex`;
      } else {
        command = `curl -fsSL https://cdn.jsdelivr.net/gh/${githubUser}/Nov@latest/install.sh | bash`;
      }
